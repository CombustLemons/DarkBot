	...
	private static String replaceDefaultDirs(String directory)
	{
		...
		while (directoryLowerCase.contains(DEFAULT_DIR))
		{
			...
			String toReplace = formattedDirectory.substring(defaultDirIndex,
					DEFAULT_DIR.length());
			formattedDirectory = replace(formattedDirectory, toReplace,
					defaultDirectory);
			directoryLowerCase = replace(directoryLowerCase, DEFAULT_DIR,
					defaultDirectory);
		}
		return formattedDirectory;
	}

	private static String replace(String modifying, String original,
			String replacement)
	{
		String modified = modifying;
		int index = modifying.indexOf(original);
		if (index > -1)
			modified = modifying.substring(0, index) + replacement
					+ modifying.substring(index + original.length());
		return modified;
	}
