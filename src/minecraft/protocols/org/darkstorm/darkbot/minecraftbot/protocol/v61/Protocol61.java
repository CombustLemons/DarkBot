...
public final class Protocol61 extends AbstractProtocol implements EventListener
{
	...
	public void onHandshake(HandshakeEvent event)
	{
		bot.getConnectionHandler().sendPacket(
				new Packet2Handshake(VERSION, event.getSession().getUsername(),
						event.getServer(), event.getPort()));
	}
	...
	{
		ConnectionHandler handler = bot.getConnectionHandler();
		handler.sendPacket(new Packet101CloseWindow(event.getInventory()
				.getWindowId()));
	}
	...
	{
		ConnectionHandler handler = bot.getConnectionHandler();
		handler.sendPacket(new Packet14BlockDig(event.isEntireStack() ? 3 : 4,
				0, 0, 0, 0));
	}
	...
	public void onEntityInteract(EntityInteractEvent event)
	{
		...
		if (event instanceof EntityHitEvent)
			...
			return;
		handler.sendPacket(new Packet7UseEntity(bot.getPlayer().getId(), event
				.getEntity().getId(), mode));
	}
	...
	{
		ConnectionHandler handler = bot.getConnectionHandler();
		handler.sendPacket(new Packet18Animation(bot.getPlayer().getId(),
				Animation.SWING_ARM));
	}
	...
	{
		ConnectionHandler handler = bot.getConnectionHandler();
		handler.sendPacket(new Packet19EntityAction(bot.getPlayer().getId(),
				event.isCrouching() ? Action.CROUCH : Action.UNCROUCH));
	}
	...
	public void onSprintUpdate(SprintUpdateEvent event)
	{
		ConnectionHandler handler = bot.getConnectionHandler();
		handler.sendPacket(new Packet19EntityAction(bot.getPlayer().getId(),
				event.isSprinting() ? Action.START_SPRINTING
	...
	public void onBlockBreak(BlockBreakEvent event)
	{
		...
		if (event instanceof BlockBreakStartEvent)
			...
			return;
		handler.sendPacket(new Packet14BlockDig(code, event.getX(), event
				.getY(), event.getZ(), event.getFace()));
	}
	...
	public void onBlockPlace(BlockPlaceEvent event)
	{
		...
		if (event.getItem() != null)
		{
			...
			if (event.getItem().getId() == 323)
				eventManager.sendEvent(new EditSignEvent(new BlockLocation(
						event.getX(), event.getY(), event.getZ())));
			else if (event.getItem().getId() == 137)
				eventManager.sendEvent(new EditCommandBlockEvent(
						new BlockLocation(event.getX(), event.getY(), event
								.getZ())));
		}
	}
	...
	public void onItemUse(ItemUseEvent event)
	{
		...
		if (placePacket.itemStack != null
				&& placePacket.itemStack.getId() == 346)
	...
	public void onPacketProcess(PacketProcessEvent event)
	{
		...
		case 1:
		{
			...
			eventManager.sendEvent(new LoginEvent(loginPacket.playerId,
					loginPacket.worldType, loginPacket.gameMode,
					loginPacket.dimension, loginPacket.difficulty,
					loginPacket.worldHeight, loginPacket.maxPlayers));
		...
		case 4:
		{
			...
			eventManager.sendEvent(new TimeUpdateEvent(timePacket.time,
					timePacket.otherTime));
			break;
		}
		case 5:
		{
			Packet5PlayerInventory inventoryPacket = (Packet5PlayerInventory) packet;
			eventManager.sendEvent(new PlayerEquipmentUpdateEvent(
					inventoryPacket.entityID, inventoryPacket.slot,
		...
		case 8:
		{
			...
			eventManager.sendEvent(new HealthUpdateEvent(
					updateHealthPacket.healthMP, updateHealthPacket.food,
					updateHealthPacket.foodSaturation));
			break;
		}
		case 9:
		{
			Packet9Respawn respawnPacket = (Packet9Respawn) packet;
			eventManager.sendEvent(new RespawnEvent(
					respawnPacket.respawnDimension, respawnPacket.difficulty,
					respawnPacket.gameMode, respawnPacket.worldType,
		...
		case 13:
		{
			...
			eventManager.sendEvent(new TeleportEvent(lookMovePacket.x,
					lookMovePacket.y, lookMovePacket.z, lookMovePacket.stance,
					lookMovePacket.yaw, lookMovePacket.pitch));
			break;
		}
		case 17:
		{
			Packet17Sleep sleepPacket = (Packet17Sleep) packet;
			eventManager.sendEvent(new SleepEvent(sleepPacket.entityID,
					sleepPacket.bedX, sleepPacket.bedY, sleepPacket.bedZ));
			break;
		}
		case 18:
		{
			...
			if (animationPacket.animation == Animation.EAT_FOOD)
				eventManager.sendEvent(new EntityEatEvent(
						animationPacket.entityId));
			break;
		}
		case 20:
		{
			Packet20NamedEntitySpawn spawnPacket = (Packet20NamedEntitySpawn) packet;
			RotatedSpawnLocation location = new RotatedSpawnLocation(
					spawnPacket.xPosition / 32D, spawnPacket.yPosition / 32D,
					spawnPacket.zPosition / 32D,
					(spawnPacket.rotation * 360) / 256F,
					(spawnPacket.pitch * 360) / 256F);
			ItemStack heldItem = new BasicItemStack(spawnPacket.currentItem, 1,
					0);
			eventManager.sendEvent(new PlayerSpawnEvent(spawnPacket.entityId,
					spawnPacket.name, heldItem, location, spawnPacket.data));
			break;
		}
		case 22:
		{
			Packet22Collect collectPacket = (Packet22Collect) packet;
			eventManager.sendEvent(new EntityCollectEvent(
					collectPacket.collectedEntityId,
					collectPacket.collectorEntityId));
			break;
		}
		case 23:
		{
			...
			if (spawnPacket.throwerEntityId != 0)
				spawnData = new ThrownObjectSpawnData(spawnPacket.type,
						...
						spawnPacket.speedX / 8000D, spawnPacket.speedY / 8000D,
						spawnPacket.speedZ / 8000D);
			else
				spawnData = new ObjectSpawnData(spawnPacket.type);
			eventManager.sendEvent(new ObjectEntitySpawnEvent(
					spawnPacket.entityId, location, spawnData));
			break;
		}
		case 24:
		{
			Packet24MobSpawn spawnPacket = (Packet24MobSpawn) packet;
			LivingEntitySpawnLocation location = new LivingEntitySpawnLocation(
					spawnPacket.xPosition / 32D, spawnPacket.yPosition / 32D,
					spawnPacket.zPosition / 32D,
					(spawnPacket.yaw * 360) / 256F,
					(spawnPacket.pitch * 360) / 256F,
					(spawnPacket.headYaw * 360) / 256F);
			LivingEntitySpawnData data = new LivingEntitySpawnData(
					spawnPacket.type, spawnPacket.velocityX / 8000D,
					spawnPacket.velocityY / 8000D,
					spawnPacket.velocityZ / 8000D);
			eventManager
					.sendEvent(new LivingEntitySpawnEvent(spawnPacket.entityId,
							location, data, spawnPacket.metadata));
			break;
		}
		case 25:
		{
			Packet25EntityPainting spawnPacket = (Packet25EntityPainting) packet;
			PaintingSpawnLocation location = new PaintingSpawnLocation(
					spawnPacket.xPosition, spawnPacket.yPosition,
					spawnPacket.zPosition, spawnPacket.direction);
			eventManager.sendEvent(new PaintingSpawnEvent(spawnPacket.entityId,
					location, spawnPacket.title));
			break;
		}
		case 26:
		{
			Packet26EntityExpOrb spawnPacket = (Packet26EntityExpOrb) packet;
			SpawnLocation location = new SpawnLocation(spawnPacket.posX / 32D,
					spawnPacket.posY / 32D, spawnPacket.posZ / 32D);
			eventManager.sendEvent(new ExpOrbSpawnEvent(spawnPacket.entityId,
					location, spawnPacket.xpValue));
			break;
		}
		case 28:
		{
			Packet28EntityVelocity velocityPacket = (Packet28EntityVelocity) packet;
			eventManager.sendEvent(new EntityVelocityEvent(
					velocityPacket.entityId, velocityPacket.motionX / 8000D,
		...
		case 33:
		{
			...
			if (packet instanceof Packet31RelEntityMove
				...
				eventManager.sendEvent(new EntityMoveEvent(
						entityPacket.entityId, entityPacket.xPosition / 32D,
						entityPacket.yPosition / 32D,
						entityPacket.zPosition / 32D));
			if (packet instanceof Packet32EntityLook
					|| packet instanceof Packet33RelEntityMoveLook)
				eventManager.sendEvent(new EntityRotateEvent(
						entityPacket.entityId, (entityPacket.yaw * 360) / 256F,
		...
		case 34:
		{
			...
			eventManager.sendEvent(new EntityTeleportEvent(
					teleportPacket.entityId, teleportPacket.xPosition / 32D,
		...
		case 35:
		{
			...
			eventManager.sendEvent(new EntityHeadRotateEvent(
					headRotatePacket.entityId,
		...
		case 38:
		{
			...
			if (statusPacket.entityStatus == 2)
				eventManager.sendEvent(new EntityHurtEvent(
						statusPacket.entityId));
			else if (statusPacket.entityStatus == 3)
				eventManager.sendEvent(new EntityDeathEvent(
						statusPacket.entityId));
			else if (statusPacket.entityStatus == 9)
				eventManager.sendEvent(new EntityStopEatingEvent(
						statusPacket.entityId));
			break;
		}
		case 39:
		{
			...
			if (attachEntityPacket.vehicleEntityId != -1)
				eventManager.sendEvent(new EntityMountEvent(
						attachEntityPacket.entityId,
						attachEntityPacket.vehicleEntityId));
			else
				eventManager.sendEvent(new EntityDismountEvent(
						attachEntityPacket.entityId));
			break;
		}
		case 40:
		{
			...
			eventManager.sendEvent(new EntityMetadataUpdateEvent(
					metadataPacket.entityId, metadataPacket.metadata));
			break;
		}
		case 43:
		{
			Packet43Experience experiencePacket = (Packet43Experience) packet;
			eventManager.sendEvent(new ExperienceUpdateEvent(
					experiencePacket.experienceLevel,
					experiencePacket.experienceTotal));
			break;
		}
		case 51:
		{
		...
		case 52:
		{
			...
			try
			{
				for (int i = 0; i < multiBlockChangePacket.size; i++)
				{
					...
					int z = word0 >> 8 & 0xf;
					int y = word0 & 0xff;
					eventManager.sendEvent(new BlockChangeEvent(id, metadata,
		...
		case 53:
		{
			...
			eventManager.sendEvent(new BlockChangeEvent(blockChangePacket.type,
					blockChangePacket.metadata, blockChangePacket.xPosition,
		...
		case 100:
		{
			...
			eventManager.sendEvent(new WindowOpenEvent(
					openWindowPacket.windowId, openWindowPacket.inventoryType,
					openWindowPacket.flag ? openWindowPacket.windowTitle : "",
		...
		case 101:
		{
			...
			eventManager.sendEvent(new WindowCloseEvent(
					closeWindowPacket.windowId));
			break;
		}
		case 103:
		{
			Packet103SetSlot slotPacket = (Packet103SetSlot) packet;
			eventManager.sendEvent(new WindowSlotChangeEvent(
					slotPacket.windowId, slotPacket.itemSlot,
		...
		case 104:
		{
			...
			eventManager.sendEvent(new WindowUpdateEvent(itemsPacket.windowId,
					itemsPacket.itemStack));
			break;
		}
		case 132:
		{
			Packet132TileEntityData tileEntityPacket = (Packet132TileEntityData) packet;
			eventManager.sendEvent(new TileEntityUpdateEvent(
					tileEntityPacket.xPosition, tileEntityPacket.yPosition,
					tileEntityPacket.zPosition, tileEntityPacket.actionType,
		...
		case 130:
		{
			...
			eventManager.sendEvent(new SignUpdateEvent(signPacket.x,
					signPacket.y, signPacket.z, signPacket.text));
			break;
		}
		case 201:
		{
			...
			if (infoPacket.isConnected)
				eventManager.sendEvent(new PlayerListUpdateEvent(
						infoPacket.playerName, infoPacket.ping));
			else
				eventManager.sendEvent(new PlayerListRemoveEvent(
						infoPacket.playerName));
			break;
		}
		case 252:
		{
	...
	public void handleServerAuthData(Packet253EncryptionKeyRequest keyRequest)
	{
		...
		if (!serverId.equals("-"))
		{
			try
			{
				...
				String hash = new BigInteger(EncryptionUtil.encrypt(serverId,
						publicKey, secretKey)).toString(16);
				if (session.isValidForAuthentication()
						&& service.isValidSession(session))
			...
			{
				connectionHandler.disconnect("Unable to authenticate: "
						+ exception);
			}
		}
			...
			int additionalBitmask, boolean addSkylight, boolean addBiomes)
	{
		...
		for (int y = 0; y < 16; y++)
		{
			...
			byte[] light = Arrays
					.copyOfRange(data, dataIndex, dataIndex + 2048);
			dataIndex += chunksChanged * 2048;
			byte[] skylight = null;
			if (addSkylight)
				skylight = Arrays
		...
		{
			ChunkLoadEvent event = new ChunkLoadEvent(x, yValues[i], z,
					allBlocks[i], allMetadata[i], allLight[i], allSkylight[i],
					biomes.clone());
	...
	public void onPacketReceived(PacketReceivedEvent event)
	{
		...
		} else if (packet instanceof Packet106Transaction)
		{
			...
			bot.getEventManager().sendEvent(
					new WindowTransactionCompleteEvent(
							transactionPacket.windowId,
							transactionPacket.shortWindowId,
							transactionPacket.accepted));
