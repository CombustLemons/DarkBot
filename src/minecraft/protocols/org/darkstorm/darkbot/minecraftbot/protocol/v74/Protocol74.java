...
public final class Protocol74 extends AbstractProtocol implements EventListener
{
	...
	public void onCrouchUpdate(CrouchUpdateEvent event)
	{
		...
		handler.sendPacket(new Packet19EntityAction(bot.getPlayer().getId(),
				event.isCrouching() ? Action.CROUCH : Action.UNCROUCH, 0));
	}
	...
	public void onPacketProcess(PacketProcessEvent event)
	{
		...
		case 3:
		{
			...
			try
			{
				...
				if (json.containsKey("text"))
					eventManager.sendEvent(new ChatReceivedEvent((String) json
							.get("text")));
		...
		case 8:
		{
			...
			eventManager
					.sendEvent(new HealthUpdateEvent(health,
							updateHealthPacket.food,
		...
		case 133:
		{
			...
			eventManager.sendEvent(new EditTileEntityEvent(editPacket.x,
					editPacket.y, editPacket.z));
			break;
		}
		case 201:
		{
