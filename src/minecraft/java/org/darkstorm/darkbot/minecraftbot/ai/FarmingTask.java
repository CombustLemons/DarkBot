...
public class FarmingTask implements Task, EventListener
{
	...
	public synchronized boolean start(String... options)
	{
		if (options.length > 0)
		{
			BlockLocation endpoint1 = new BlockLocation(
					Integer.parseInt(options[0]), Integer.parseInt(options[1]),
					Integer.parseInt(options[2]));
			BlockLocation endpoint2 = new BlockLocation(
	...
	public synchronized void run()
	{
								...
								&& !isChestCovered(chest))
						{
							...
							for (BlockLocation walk : surrounding)
							{
								if (BlockType.getById(world.getBlockIdAt(walk))
										.isSolid()
										|| (BlockType.getById(
												world.getBlockIdAt(walk.offset(
														0, 1, 0))).isSolid() && BlockType
												.getById(
														world.getBlockIdAt(walk
																.offset(0, -1,
																		0)))
												.isSolid()))
									continue;
							...
							BlockLocation closestWalkOffset = closestWalk
									.offset(0, -1, 0);
									...
									.isSolid())
							{
								...
								if (originalWalk.getY()
										- closestWalkOffset.getY() > 5)
									continue chestLoop;
								closestWalkOffset = closestWalkOffset.offset(0,
										-1, 0);
							}
			...
			} else if (storageAction.equals(StorageAction.SELL))
			{
				...
				signLoop: for (BlockLocation sign : signs)
				{
					...
					if (text[0].contains("[Sell]"))
						for (int id : FARMED_ITEMS)
							if (text[2].equals(Integer.toString(id))
									&& inventory.getCount(id) >= 64)
								found = true;
					if (!found)
						continue;
					...
					BlockLocation[] surrounding = new BlockLocation[] {
							sign.offset(0, 1, 0), sign.offset(-1, 0, 0),
							sign.offset(1, 0, 0), sign.offset(0, 0, -1),
							sign.offset(0, 0, 1), sign.offset(0, -1, 0) };
					BlockLocation closestWalk = null;
					int closestDistance = Integer.MAX_VALUE;
					int face = 0;
					for (BlockLocation walk : surrounding)
		...
		if (id == 115 || id == 59 || id == 83 || id == 86 || id == 103)
		{
			...
			else if (id == 86 || id == 103)
			{
				...
				while (!BlockType
						.getById(world.getBlockIdAt(closestWalkOffset))
						.isSolid())
				{
					closestWalk = closestWalkOffset;
					if (originalWalk.getY() - closestWalkOffset.getY() > 5)
	...
	private boolean isChestCovered(BlockLocation chest)
	{
		if (checkChest(chest))
			return true;
		BlockLocation[] surrounding = new BlockLocation[] {
				chest.offset(-1, 0, 0), chest.offset(1, 0, 0),
				chest.offset(0, 0, -1), chest.offset(0, 0, 1) };
		for (BlockLocation otherChest : surrounding)
			if (bot.getWorld().getBlockIdAt(otherChest) == 54
					&& checkChest(otherChest))
				return true;
		return false;
	}
	...
	private boolean checkChest(BlockLocation chest)
	{
		...
		if (!BlockType.getById(idAbove).isSolid())
			return false;
		BlockType[] noncovers = new BlockType[] { BlockType.CHEST,
				BlockType.ENDER_CHEST, BlockType.STEP, BlockType.BED_BLOCK,
				BlockType.ANVIL, BlockType.BREWING_STAND, BlockType.WOOD_STEP,
				BlockType.WOOD_STAIRS, BlockType.BRICK_STAIRS,
				BlockType.COBBLESTONE_STAIRS, BlockType.NETHER_BRICK_STAIRS,
				BlockType.SANDSTONE_STAIRS, BlockType.SMOOTH_STAIRS };
	...
	private BlockLocation getClosestFarmable(int radius)
	{
		...
		for (int x = region != null ? region.getX() - ourLocation.getX()
				: -radius; x < (region != null ? region.getX()
				+ region.getWidth() - ourLocation.getX() : radius); x++)
		{
			for (int y = region != null ? region.getY() - ourLocation.getY()
					: -radius / 2; y < (region != null ? region.getY()
					+ region.getHeight() - ourLocation.getY() : radius / 2); y++)
			{
				for (int z = region != null ? region.getZ()
						- ourLocation.getZ() : -radius; z < (region != null ? region
						.getZ() + region.getLength() - ourLocation.getZ()
						: radius); z++)
				{
