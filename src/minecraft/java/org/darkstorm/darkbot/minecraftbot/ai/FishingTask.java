...
public class FishingTask implements Task, EventListener
{
	...
	private void useFishingRod()
	{
		...
		eventManager.sendEvent(new ItemUseEvent(player.getInventory()
				.getCurrentHeldItem()));
	}
	...
	public void onEntityVelocity(EntityVelocityEvent event)
	{
		...
		if (entity == null
				|| !(entity instanceof FishingBobEntity)
				|| !(((FishingBobEntity) entity).getThrower() instanceof MainPlayerEntity))
			return;
		System.out.println("Fishing bob velocity!");
		if (event.getVelocityX() == 0 && event.getVelocityY() < 0
				&& event.getVelocityZ() == 0)
		{
			if (fishing)
			{
				if (!switchToFishingRod())
