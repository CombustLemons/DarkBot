...
public final class BlockArea implements Cloneable
{
	...
	public BlockArea(BlockLocation corner1, BlockLocation corner2)
	{
		this(Math.min(corner1.getX(), corner2.getX()), Math.min(corner1.getY(),
				corner2.getY()), Math.min(corner1.getZ(), corner2.getZ()), Math
				.max(corner1.getX(), corner2.getX())
				- Math.min(corner1.getX(), corner2.getX()), Math.max(
				corner1.getY(), corner2.getY())
				- Math.min(corner1.getY(), corner2.getY()), Math.max(
	...
	{
		this(area.getX(), area.getY(), area.getZ(), area.getWidth(), area
				.getHeight(), area.getLength());
	}
	...
	public BlockArea(int x, int y, int z, int width, int height, int length)
	{
		...
		center = new BlockLocation(x + width / 2, y + height / 2, z + length
				/ 2);
	...
	{
		return new BlockLocation(x + (int) (Math.random() * width), y
				+ (int) (Math.random() * height), z
	...
	public boolean contains(int x, int y, int z)
	{
		return x >= this.x && y >= this.y && x < this.x + width
				&& y < this.y + height && z >= this.z && z < this.z + length;
	}
