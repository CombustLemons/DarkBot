...
public class MainPlayerEntity extends PlayerEntity
{
	...
	public void face(BlockLocation target, int face)
	{
		...
		double x = offset.getX() + ((target.getX() - offset.getX()) / 2.0D)
				+ 0.5;
		double y = offset.getY() + ((target.getY() - offset.getY()) / 2.0D);
		double z = offset.getZ() + ((target.getZ() - offset.getZ()) / 2.0D)
				+ 0.5;
		face(x, y, z);
	}
	...
	private float getRotationY(double x, double y, double z)
	{
		...
		double dis2 = Math.sqrt(Math.pow(x - this.x, 2)
				+ Math.pow(z - this.z, 2));
	...
	{
		int id = world.getBlockIdAt((int) Math.floor(x),
	...
	public void setCrouching(boolean crouching)
	{
		if (crouching != this.crouching)
		{
			...
			world.getBot().getEventManager()
					.sendEvent(new CrouchUpdateEvent(crouching));
		}
	}
	...
	public void setSprinting(boolean sprinting)
	{
		if (sprinting != this.sprinting)
		{
			...
			world.getBot().getEventManager()
					.sendEvent(new SprintUpdateEvent(sprinting));
		}
	}
