...
public class MinecraftBot extends Bot implements EventListener
{
			...
			throws AuthenticationException, UnsupportedProtocolException
	{
		...
		{
			ProtocolProvider provider = ProtocolProvider.getProvider(botData
					.getProtocol());
			if (provider == null)
				throw new UnsupportedProtocolException(
						"No protocol support for v" + botData.getProtocol()
								+ " found.");
			protocol = provider.getProtocolInstance(this);
		} else
			protocol = ProtocolProvider.getLatestProvider()
					.getProtocolInstance(this);
		...
		if (botData.getSession() != null
				&& !authService.isValidSession(botData.getSession()))
		...
		if (botData.getHttpProxy() != null)
			...
			loginProxy = null;
		if (StringUtils.isNotBlank(botData.getPassword())
				&& (botData.getSession() == null || !botData.getSession()
						.isValidForAuthentication()))
		{
			try
			{
				session = authService.login(botData.getUsername(),
						botData.getPassword(), loginProxy);
		...
		eventManager.sendEvent(new HandshakeEvent(session, connectionHandler
				.getServer(), connectionHandler.getPort()));
		new TickHandler();
	}
	...
	{
		setWorld(new BasicWorld(this, event.getWorldType(),
				event.getDimension(), event.getDifficulty(),
				event.getWorldHeight()));
		player = new MainPlayerEntity(world, event.getPlayerId(),
				session.getUsername(), event.getGameMode());
	...
	{
		setWorld(new BasicWorld(this, event.getWorldType(),
				event.getRespawnDimension(), event.getDifficulty(),
				event.getWorldHeight()));
	...
	public void onWindowOpen(WindowOpenEvent event)
	{
		if (player == null)
			return;
		System.out.println("Opened inventory " + event.getInventoryType()
				+ ": " + event.getSlotCount() + " slots");
		switch (event.getInventoryType())
		{
		case CHEST:
			player.setWindow(new ChestInventory(this, event.getWindowId(),
					event.getSlotCount() == 27 ? false : true));
			break;
		default:
		}
	}
	...
	public void onWindowSlotChange(WindowSlotChangeEvent event)
	{
		...
		if (event.getWindowId() != 0
				&& (window == null || event.getWindowId() != window
						.getWindowId()))
			return;
		if (event.getWindowId() == 0)
			player.getInventory().setItemFromServerAt(event.getSlot(),
					event.getNewItem());
		else
	...
	public synchronized void updateMovement()
	{
		...
		if (move && rotate)
			event = new PlayerMoveRotateEvent(player, x, y, z, yaw, pitch,
					onGround);
		else if (move)
	...
	public synchronized void setWorld(World world)
	{
		...
		{
			if (world != null)
				player = new MainPlayerEntity(world, player.getId(),
