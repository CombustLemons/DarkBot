...
public abstract class ProtocolProvider
{
	...
	private static final List<ProtocolProvider> loadProviders()
	{
		...
		ServiceLoader<ProtocolProvider> providerLoader = ServiceLoader.load(
				ProtocolProvider.class, classLoader);
		loop: for (ProtocolProvider provider : providerLoader)
		{
			for (ProtocolProvider installed : providers)
				if (provider.getSupportedVersion() == installed
						.getSupportedVersion())
					continue loop;
	...
	public static final ProtocolProvider getProvider(int version)
	{
		...
		try
		{
			String className = "org.darkstorm.darkbot.minecraftbot.protocol.v"
					+ version + ".Protocol" + version + "$Provider";
			Class<? extends ProtocolProvider> providerClass = Class.forName(
					className).asSubclass(ProtocolProvider.class);
	...
	public static final ProtocolProvider getLatestProvider()
	{
		...
		for (ProtocolProvider provider : providers)
			if (latestProvider == null
					|| provider.getSupportedVersion() > latestProvider
							.getSupportedVersion())
