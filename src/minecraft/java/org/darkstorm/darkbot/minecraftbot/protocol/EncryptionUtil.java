...
public final class EncryptionUtil
{
	...
	public static byte[] encrypt(String string, PublicKey publicKey,
			SecretKey secretKey) throws NoSuchAlgorithmException,
			UnsupportedEncodingException
	{
		return hash("SHA-1", string.getBytes("ISO_8859_1"),
				secretKey.getEncoded(), publicKey.getEncoded());
	}
	...
	public static SecretKey createSecretKey(PrivateKey key, byte[] data)
			throws GeneralSecurityException
	{
		return new SecretKeySpec(cipher(2, key, data), "AES");
	}

	public static byte[] cipher(int method, Key key, byte[] data)
			throws GeneralSecurityException
	{
	...
	public static OutputStream encryptOutputStream(OutputStream outputStream,
			SecretKey key)
	{
		return new CipherOutputStream(outputStream,
				createBlockCipher(key, true));
	}
	...
	public static BufferedBlockCipher createBlockCipher(SecretKey key,
			boolean out)
	{
		...
		blockCipher.init(
				out,
				new ParametersWithIV(new KeyParameter(key.getEncoded()), key
						.getEncoded(), 0, 16));
		return blockCipher;
	}
}