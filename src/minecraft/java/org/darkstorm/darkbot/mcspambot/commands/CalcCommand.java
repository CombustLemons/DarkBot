		...
		for (char c : expression.toCharArray())
		{
			...
			if (Character.isDigit(c) || (c == '.' && !string.contains(".")))
			{
				if (!string.isEmpty())
				{
					try
					{
						Double.parseDouble(string.endsWith(".") ? string
			...
			} else if (c == '(' || c == ')')
			{
				if (!string.isEmpty())
				{
					...
					{
						double d = Double
								.parseDouble(string.endsWith(".") ? string
				...
				if (parenthesis == null)
					throw new RuntimeException(
							"Parenthesis not found: should not happen.");
			...
			} else if (!string.isEmpty())
			{
				try
				{
					double d = Double.parseDouble(string.endsWith(".") ? string
							.substring(0, string.length() - 1) : string);
					buffer = new StringBuffer();
					exprQueue.add(new Variable(d));
				} catch (NumberFormatException exception)
	...
	private Queue<StackItem> toReversePolishNotation(Queue<StackItem> expression)
	{
		...
		while (expression.size() > 0)
		{
			...
			} else if (stack == Parenthesis.OPEN)
			{
				...
				queue.offer(new Variable(
						compute(toReversePolishNotation(subExpr))));
