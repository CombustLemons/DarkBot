...
public class HelpCommand extends AbstractCommand
{
	...
	public HelpCommand(DarkBotMC MCBot)
	{
		super(
				MCBot,
				"help",
				"Displays OptionDescription and Description for all registered commands",
				"[command]", ".*");
	}
		...
		if (args.length == 0)
		{
			...
			for (Command c : getBot().getCommandManager().getCommands())
			{
				...
				if (commandUsage < c.getOptionDescription().length())
					commandUsage = Math.max(
							(c.getOptionDescription().length() - 13), 1);
				if (commandDesc < c.getDescription().length())
					commandDesc = Math.max((c.getDescription().length() - 19),
							1);
			}
			System.out.println();
			...
			for (Command c : getBot().getCommandManager().getCommands())
			{
				...
				System.out.println((String.format("%-" + (commandName + 13)
						+ "s%-" + (commandUsage + 14) + "s%s\n",
						"| !" + c.getName(), " | " + c.getOptionDescription(),
						" | " + c.getDescription())));
			}
			System.out.println();
			...
			for (Command c : getBot().getCommandManager().getCommands())
				if (c.getName().equalsIgnoreCase(args[0]))
					bot.say("!" + c.getName() + " " + c.getOptionDescription()
							+ " - " + c.getDescription());
	}
