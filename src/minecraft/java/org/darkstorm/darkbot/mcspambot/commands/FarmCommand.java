...
public class FarmCommand extends AbstractCommand
{
	...
	public FarmCommand(MinecraftBotWrapper bot)
	{
		super(bot, "farm", "Activate the farming task",
				"[<x1> <y1> <z1> <x2> <y2> <z2>] [STORE|sell]",
				"(?i)([=]?[-]?[0-9]+( [=]?[-]?[0-9]+){5})?( (sell|store))?");
	}
	...
	public void execute(String[] args)
	{
		...
		if (args.length > 0
				&& (args[args.length - 1].equalsIgnoreCase("store") || args[args.length - 1]
						.equalsIgnoreCase("sell")))
		{
			task.setStorageAction(args[args.length - 1]
					.equalsIgnoreCase("store") ? StorageAction.STORE
					: StorageAction.SELL);
			length--;
		}
