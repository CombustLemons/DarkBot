...
public class DarkBotMC extends MinecraftBotWrapper
{
	...
	public static void main(String[] args)
	{
		...
		if (options.has(protocolsOption))
		{
			...
			System.out
					.println("If no protocol is specified, it wil take the latest protocol version, assuming there are protocols listed above.");
			return;
		}
		...
		if (!offline)
		{
			...
			user: do
			{
				...
				while (true)
				{
					...
					try
					{
						session = authService.login(accountParts[0],
								accountParts[1],
				...
				System.out
						.println("[" + session.getUsername() + "] " + session);
				...
				while (true)
				{
					...
					try
					{
						DarkBotMC bot = new DarkBotMC(generateData(server,
								session.getUsername(), session.getPassword(),
								authService, session, protocol, null, proxy),
								owner);
		...
		} else
		{
			while (true)
			{
				...
				try
				{
					...
					if (username == null)
						name = Util
								.generateRandomString(10 + random.nextInt(6));
					else
						name = username;
					DarkBotMC bot = new DarkBotMC(generateData(server, name,
							null, null, null, protocol, null, proxy), owner);
					while (bot.getBot().isConnected())
					{
						try
						{
